...
function EditInvoiceModal({
  ...
  // Only mount the form when the modal is open and an invoice exists.
...
function EditInvoiceForm({
  ...
  // This component is only mounted when an invoice is present; safe to initialize hooks from props.
  const [localInvoice, setLocalInvoice] = useState<AdminInvoice>(invoice);

  const handleChange = <K extends keyof AdminInvoice>(
    key: K,
    value: AdminInvoice[K],
  ) => {
    // prev is guaranteed to be an AdminInvoice here
    setLocalInvoice((prev) => ({ ...prev, [key]: value }) as AdminInvoice);
  };
          ...
          <div className="form-row">
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={(e) =>
                  handleChange("type", e.target.value as InvoiceType)
                }
              >
